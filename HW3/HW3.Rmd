...
#Packages ########################### check for delete some librsry

###########################################    progit átdolgozni 

###########################################     summary-table funk lehet kell abbol info szöveg íráshoz

################################################## helper function_t betenni

###################################################### ggplotokról írni lehet az csak masik report

########################################################## set seed- et at allítani 
...
ggplot( data = data , aes( x = fast_growth ) ) +
  geom_histogram( aes( y = ..count.. / sum( count ) ) , size = 0.1 , fill = 'darkgreen',
                  bins = 3)+
...
# Modelling
The analysis aims to predict the fast growth of companies. To be able to predict the yearly CAGR calculated between 2012 and 2013 for each company. The threshold was set for a 30% increase in CAGR, and only those firms who reach it are considered fast-growing companies. In comparison, at the end of the data cleaning, there were almost 2 500 fast-growing companies, which is almost 20% of the total. If the analysis would consider only 1 year of CAGR, that would be a too short period to help the prediction. However, the prediction aims to analyze the firms for the 2 years only. 
...
train_control <- trainControl(
  ...
  number = 5,
  classProbs = TRUE,
  summaryFunction = twoClassSummaryExtended, ###################################### helper-ben van
